...
public class SignTest {
    ...
    public void signTest() {
        try {
            ...
            FileInputStream fis2 = new FileInputStream("C:\\Users\\fta\\.android\\debug.keystore");
            KeyStore ks = KeyStore.getInstance("JKS"); // 加载证书库
            char[] kspwd = "android".toCharArray(); // 证书库密码
            char[] keypwd = "android".toCharArray(); // 证书密码
            String alias = "androiddebugkey";// 别名
     ...
     * 加密/解密
     *
     * @param key    私钥打包成byte[]形式
     * @param data   要解密的数据
     * @param opmode 操作类型（Cipher.DECRYPT_MODE为解密，Cipher.ENCRYPT_MODE为加密）
     * @return 解密数据
     */
    public byte[] crypt(Key key, byte[] data, int opmode) {
        try {
